...
fn get_paging_object<T: DeserializeOwned>(
    url: &str,
    access_token: &str,
    refresh_token: &str,
) -> PagingObject<T> {
...
impl<T: DeserializeOwned + Clone> PagingObject<T> {
    pub fn get_next(&self, access_token: &str, refresh_token: &str) -> Option<PagingObject<T>> {
        let url = self.next.clone()?;

        Some(get_paging_object(&url, access_token, refresh_token))
    }

    pub fn get_previous(&self, access_token: &str, refresh_token: &str) -> Option<PagingObject<T>> {
        let url = self.previous.clone()?;
    ...
    pub fn get_all_items(&self, access_token: &str, refresh_token: &str) -> Vec<T> {
        let mut previous_items: Vec<T> = Vec::new();
        let mut previous = self.get_previous(access_token, refresh_token);
        while let Some(p) = previous {
            let mut items: Vec<T> = p.get_items();
            items.reverse();
            previous_items.append(&mut items);
            previous = p.get_previous(access_token, refresh_token);
        }

        previous_items.reverse();
        let mut current_items: Vec<T> = self.get_items();
        previous_items.append(&mut current_items);

        let mut next = self.get_next(access_token, refresh_token);
        while let Some(n) = next {
            let mut items: Vec<T> = n.get_items();
            previous_items.append(&mut items);
            next = n.get_next(access_token, refresh_token);
        }
